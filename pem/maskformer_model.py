...
class MaskFormer(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        losses = ["labels", "masks"]
        if cfg.MODEL.MASK_FORMER.USE_EDGE_LOSS:
            losses.append("edge")
            weight_dict["loss_edge"] = cfg.MODEL.SEM_SEG_HEAD.EDGE_LOSS_WEIGHT
        print(f'损失构成{losses}')
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_image in targets:
            ...
            # ➕ 添加 edge 图（基于掩码生成边缘）
            edge_target = self.generate_edge_from_mask(padded_masks)

            new_targets.append(
                {
                    "labels": targets_per_image.gt_classes,
                    "masks": padded_masks,
                    "edge": edge_target[None, ...],  # 加入边缘图
                }
            )
        return new_targets

    def generate_edge_from_mask(self, masks):
        """
        masks: [N, H, W] binary masks
        return: [H, W] binary edge map
        """
        device = masks.device
        edge_map = torch.zeros_like(masks[0], dtype=torch.float32)
        ...
        for mask in masks:
            ...
            edge = (grad > 0.1).float()[0, 0]
            edge_map = torch.max(edge_map, edge)  # merge all instance edges

        return edge_map  # shape: [H, W]
